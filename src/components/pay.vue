<template>
  ...
  <div>
    ...
    <div class="mui-page-content" style="margin-top: 2.1875rem;">
      <div class="mui-scroll-wrapper">
        <div class="mui-scroll">
          ...
          <ul class="mui-table-view mui-table-view-chevron">
            <li class="mui-table-view-cell">
              <a class="">订单金额<i class="mui-pull-right update">￥{{Hair.newPrice}}</i></a>
            </li>
            <li class="mui-table-view-cell">
    ...
    <div class="calBanner">
      <span class="left">
        <span style="color: orangered;font-weight: bold;">合计：￥{{Hair.newPrice}}
    ...
    methods: {
      getUserInfo: function() {
        ...
        this.$http({
          ...
          that.userInfo = res.data.list[0]
        }).catch(function(err) {
          that.$mui.toast('获取用户信息失败')
          console.log(err)
        })
      },
      getTHairById: function(orderId) {
        this.$http('http://localhost:8080/getTHairById/' + orderId).then(res => {
          console.log(res)
      ...
      fakePay: function() {
        ...
        var that = this
        var btnArray = ['否', '是'];
        this.$mui.confirm('需付￥'+that.Hair.newPrice, '支付', btnArray, function(e) { /* +that.orderHead.totalPrice */
          if (e.index == 1) {
            that.$mui.toast('支付成功')
            that.$router.push('/')
          } else {
